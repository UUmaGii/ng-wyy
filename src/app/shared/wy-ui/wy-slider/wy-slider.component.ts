import { Component, OnInit, ViewEncapsulation, ViewChild, ElementRef, Input, Inject, ChangeDetectionStrategy, ChangeDetectorRef, OnDestroy, forwardRef, Output, EventEmitter } from '@angular/core';
...
export class WySliderComponent implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  private createDraggingObservables(){
    ...
    const mouse: SliderEventObserverConfig = {
      ...
      filterFunc: (e: Event) => e instanceof MouseEvent,
    ...
    const touch: SliderEventObserverConfig = {
      ...
      filterFunc: (e: Event) => e instanceof TouchEvent,
    ...
    [mouse, touch].forEach(source => {
      ...
      source.startPlucked$ = fromEvent(this.sliderDom, start).pipe(
        filter(filterFunc), // 过滤掉没有对应的事件
        tap(sliderEvent), // 阻止事件的冒泡跟默认事件
        pluck(...pluckKey), // 选择事件发出
  ...
  private subscribeDrag(events: string[] = ['start', 'move', 'end']){
    ...
    if (inArray(events, 'end') && this.dragStart$ && !this.dragEnd_){
  ...
  // 判断是不是NAN
  private assetValueValid(value: SliderValue): boolean{
